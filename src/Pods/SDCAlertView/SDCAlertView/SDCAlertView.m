...
- (void)positionSelf {
    ...
    if (self.alertContentView.textFields.count > 0 && [self.alertContentView.textFields[0] isFirstResponder]) {
        CGFloat yOffset = self.superview.center.y - ((self.superview.frame.size.height - 216) / 2 - self.frame.size.height / 2);
        [self sdc_centerInSuperviewWithOffset:UIOffsetMake(0, yOffset * -1)];
    } else {
