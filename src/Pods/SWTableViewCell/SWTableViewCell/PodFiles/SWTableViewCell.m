...
- (void)initializer
{
    ...
    for (NSInteger i = constraints.count - 1; i >= 0; i--) {
        ...
        if ([firstItem isDescendantOfView:self.scrollViewContentView]) {
            if (NO == [secondItem isDescendantOfView:self.scrollViewContentView]) {
                secondItem = self.scrollViewContentView;
            }
        }
        else if ([secondItem isDescendantOfView:self.scrollViewContentView]) {
            if (NO == [firstItem isDescendantOfView:self.scrollViewContentView]) {
        ...
        if (shouldMoveToContentView) {
            ...
            [self.scrollViewContentView addConstraint:contentViewConstraint];
        }
    }
